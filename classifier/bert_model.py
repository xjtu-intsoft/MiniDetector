...
if __name__ == '__main__':
    ...
    raw_data = pd.read_csv("labelSentences.csv", encoding="utf_8_sig")
    ...
    for train_index, test_index in kf.split(raw_data):
        ...
        # 产生输入句 数据
        train_data = processor.get_input(train, max_seq_len)
        test_data = processor.get_input(test, max_seq_len)
        ...
        # 训练和测试的样本数量
        total_train_batch = math.ceil(len(train) / batch_size)
        total_test_batch = math.ceil(len(test) / batch_size)
        ...
        for epoch in range(4):
            ...
            result = evaluate_accuracy(test_iter, model, device,total_test_batch)
            print('epoch %d, loss %.4f, train acc %.3f, time: %.3f' %
                (epoch + 1, train_loss_sum / n, train_acc_sum / n, (time.time() - start)))
            print(result)

        torch.save(model, 'fine_tuned_chinese_bert_%s.bin' % model_index)