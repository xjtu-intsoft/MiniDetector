...
def getPolicyCompareData(mpDir):
    # mpDir = r"D:\WORK\2023\PAPER\TSE24\data\policy-3k\121创投"
    policyData = json.load(open(mpDir, "r", encoding="utf-8"))[0]
    # print(policyData)
    
    bestMethodData = policyData["threshold3"]
...
def sumBehavior():
    ...
    # sankeyFlowDataFrame = pandas.DataFrame(columns=["startpoint", "type", "usage", "number"])
    # for sf in sumFlowDict:
    #     # print(sf)
    #     sankeyFlowDataFrame.loc[sankeyFlowDataFrame.shape[0]] = {
    #         "startpoint": eval(sf)[0],
    #         "type": eval(sf)[1],
    #         "usage": eval(sf)[2],
    #         # "endpoint": eval(sf)[3],
    #         "number": sumFlowDict[sf]
    #     }
    # sankeyFlowDataFrame.to_csv(r"D:\WORK\2023\PAPER\TSE24\data\consistency-3k\flowSankey.csv", index=False)
    
    json.dump(sumSankeyNode, open(r"D:\WORK\2023\PAPER\TSE24\data\consistency-3k\flowSankeyNodes-2.json", "w", encoding="utf-8"), ensure_ascii=False)
    json.dump(sumSankeyLink, open(r"D:\WORK\2023\PAPER\TSE24\data\consistency-3k\flowSankeyLinks-2.json", "w", encoding="utf-8"), ensure_ascii=False)

    
    practiceDictToHeatMapData(dictData=sumCodePracticeDict, csvPath=r"D:\WORK\2023\PAPER\TSE24\data\consistency-3k\codePracticeHeatmap.csv")
    
    practiceDictToHeatMapData(dictData=sumDocPracticeDict, csvPath=r"D:\WORK\2023\PAPER\TSE24\data\consistency-3k\policyPracticeHeatmap.csv")
    
    

# mpDir = r"D:\WORK\2023\PAPER\TSE24\data\behavior-3k-v2\王卡申请助手\behavior(processed).pickle"
# getBehaviorCompareData(mpDir)
